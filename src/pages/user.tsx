...
import { useDelUserrolMutation, useSetRolUserMutation, useUserDeleteMutation, useUserGetQuery, useUserRegisMutation } from "@/rtk/adminSl";
    ...
    <div className="space-y-8">
      ...
      <div className="flex xl:flex-row items-start xl:items-center justify-between gap-6">
        <div className="flex flex-wrap gap-4 xl:w-[50%] sm:w-[70%]">
          <div className="flex items-center flex-1 min-w-[220px] px-4 py-2 rounded-lg shadow-md
          ...
          <Select onValueChange={(val) => setSel(val)} >
            <SelectTrigger className="w-[180px] h-[44px] rounded-lg
      ...
      <div className="overflow-x-auto w-full" style={{ scrollbarColor: "transparent transparent " }} >
        <table className="min-w-[700px] w-full border-collapse">
          <thead className="bg-gray-50 dark:bg-[#171227FF]">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm sm:text-base font-medium text-gray-500">Date</th>
              <th className="px-4 py-3 text-left text-sm sm:text-base font-medium text-gray-500">Payment status</th>
              <th className="px-4 py-3 text-left text-sm sm:text-base font-medium text-gray-500">Number</th>
              <th className="px-4 py-3 text-left text-sm sm:text-base font-medium text-gray-500">Action</th>
            </tr>
          </thead>
                  ...
                  >
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setArr((prev) => prev.filter((id) => id !== user?.userId));
                          }
                        }}
                      />
                      <img
                        src={`http://37.27.29.18:8007/images/${user?.image}`}
                        alt="User avatar"
                    ...
                    <td className="px-4 py-3 text-sm sm:text-base text-gray-600 dark:text-gray-400">
                      {["SuperAdmin", "Admin", "User"].map((roleName) => {
                        const hasRole = user?.userRoles.some((r: any) => r?.name === roleName);

                        const handleRoleChange = () => {
                          if (roleName === "Admin") {
                            rolUsDel({ idUs: user?.userId, rolUs: "34808f49-52e9-4fb7-9001-cf05800d608d" });
                            EditRole({ Uid: user?.userId, Rid: "82f1e62b-03ca-4d0e-a61a-e5398d8a67e1" });
                          } else if (roleName === "User") {
                            rolUsDel({ idUs: user?.userId, rolUs: "82f1e62b-03ca-4d0e-a61a-e5398d8a67e1" });
                            EditRole({ Uid: user?.userId, Rid: "34808f49-52e9-4fb7-9001-cf05800d608d" });
                    ...
                    <td className="px-4 py-3 text-sm sm:text-base text-gray-600 dark:text-gray-400">
                      <button
                        onClick={() => {
                          delUs(user?.userId), setLoadDel(user?.userId), toast(`${user?.userName}`);
                        }}
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-center" >
          {totalPages > 1 && (
            <div className="mt-6 overflow-x-auto max-w-[900px] " style={{ scrollbarColor: "transparent transparent " }} >
